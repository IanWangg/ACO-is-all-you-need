...
from utils.dataset import CarlaDataset
from utils.loss import LabelMoCoLoss, SeqLabelMoCoLoss
...
parser = argparse.ArgumentParser(description='PyTorch ACO Carla Training with Multiple Future Actions as Labels')
parser.add_argument('--data', 
                    type=str, 
                    default='/home/ywang3/workplace/dataset_carla/town1_large/datasets_train/cilrs_datasets_train',
                    help='path to training dataset')
parser.add_argument('--preloads', 
                    type=str,
                    default='/home/ywang3/workplace/dataset_carla/town1_large/_preloads/datasets_train.npy',
                    help='path to training preloads npy file')
parser.add_argument('--eval-data', 
                    type=str, 
                    default='/home/ywang3/workplace/dataset_carla/town1_small/datasets_train/cilrs_datasets_train',
                    help='path to eval dataset')
parser.add_argument('--eval-preloads', 
                    type=str,
                    default='/home/ywang3/workplace/dataset_carla/town1_small/_preloads/datasets_train.npy',
                    help='path to eval preloads npy file')
...
# carla and sequential label configs:
parser.add_argument('--exp-name', default=None, type=str)
parser.add_argument('--action-seq-length', default=1, type=int,
                    help='length of future actions as labels (default: 10)')
parser.add_argument('--interval', default=10, type=int,
                    help='Gap between frames, for eliminating correlation between frames (default: 10)')
parser.add_argument('--use-throttle', default=False, type=bool,
                    help='Whether use throttle as part of the action (default: False)')
parser.add_argument('--action-mask', default='infnorm', type=str, choices=['infnorm', '1norm'])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print(args.exp_name, flush=True)
    # exp_name = f"td_aco_large_res18_infnorm actio_seq_length-{args.action_seq_length}"
    if args.exp_name is not None:
        exp_name = args.exp_name
    else:
        exp_name = f'ACO_seq{args.action_seq_length}_interval{args.interval}_model{args.model}_mask{args.action_mask}'
        
    print(f'Running Experiment : {exp_name}', flush=True)
    wandb.init(project=exp_name, entity="yiranwang1027")
    args.gpu = gpu
    ...
    model = moco.builder.SeqLabelMoCo(
        models.__dict__[args.arch],
        args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.action_seq_length, args.mlp, args.ckpt_dir)
    ...
    criterion = SeqLabelMoCoLoss(channel=channel, T=args.moco_t, device=device_name, thres=args.thres, action_mask=args.action_mask)
    ...
    # Data loading code
    traindir = args.data
    train_preloads = args.preloads
    evaldir = args.eval_data
    eval_preloads = args.eval_preloads
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
    print('Start Setting Up Augmentations', flush=True)
    if args.aug_plus:
    ...
    print('Start Setting Up Dataset', flush=True)
    train_dataset = CarlaDataset(
        root_dir=traindir,
        preloads=train_preloads,
        interval=args.interval, 
        action_seq_length=args.action_seq_length,
        transform=True,
        aug=moco.loader.TwoCropsTransform(transforms.Compose(augmentation))
    )
    memory_dataset = CarlaDataset(
        root_dir=traindir,
        preloads=train_preloads,
        interval=args.interval, 
        action_seq_length=args.action_seq_length,
        transform=True,
        aug=transforms.Compose(test_aug)
    )
    eval_dataset = CarlaDataset(
        root_dir=evaldir,
        preloads=eval_preloads,
        interval=args.interval, 
        action_seq_length=args.action_seq_length,
        transform=True,
        aug=transforms.Compose(test_aug)
    ...
    print('Preparing Loaders', flush=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename=f'{exp_name}/checkpoint_{epoch:04d}.pth.tar')
...
def test(eval_loader, memory_loader, model, epoch, args):
    ...
    with torch.no_grad():
        for images, labels in tqdm(memory_loader, desc='feature extracting'):
            ...
            _, feat = model.extract_feature(images)
        ...
        for images, labels in test_bar:
            ...
            labels = labels.reshape(pred_labels)
            top1 += (torch.abs(pred_labels - labels.squeeze()) < 0.08).float().sum().item()
            test_bar.set_description('Test Epoch: [{}/{}] Acc@1:{:.2f}%'.format(epoch, args.epochs, top1 / total_num * 100))

    print(f'eval-top1 : {top1 / total_num * 100}')
...
def train(train_loader, model, criterion, optimizer, epoch, args, cnt):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            # images[0] = images[0].cuda(0)
            # images[1] = images[1].cuda(0)
        ...
        if i % args.log_img_freq == 0:
            ...
            # log_dict['img'] = logimg
        print(f'Epoch {epoch} : loss {loss}, ins_loss {loss_list[0].item()}, act_loss {loss_list[1].item()}')
        wandb.log(log_dict)
