...
class LabelMoCoLoss(nn.Module):
    ...
    def forward(self, q, k, label, queue_k, queue_label):
        ...
        # 1. second mask, action label mask # maybe change to pointwise residual?
        # the following line is the 1-norm threshold
...
class SeqLabelMoCoLoss(nn.Module):
    def __init__(self, T, device, channel, thres, action_mask):
        super().__init__()
        self.T = T
        self.device = device
        self.channel = channel
    ...
    def forward(self, q, k, label, queue_k, queue_label):
        ...
        # 1. second mask, action label mask # maybe change to pointwise residual?
        # print(label.shape, queue_label.shape)
        label_difference = label.unsqueeze(-1) - queue_label.unsqueeze(-1).T
        label_difference = label_difference.permute(0, 2, 1)
        
        # 2-norm mask
        if self.action_mask == '1norm':
            mask_action_label = (torch.norm(label_difference, p=1, dim=-1) < self.thres).float().to(self.device)
        
        # inf-norm mask
        elif self.action_mask == 'infnorm':
            mask_action_label = (torch.abs(label_difference).max(dim=-1, keepdim=True)[0].squeeze(-1) < self.thres).float().to(self.device)
